...
export class CopilotIntegration {
  ...
  public async generateCodeForTask(
    ...
    if (isAutomaticMode && !autoComplete) {
      const isCompleted = await this.handleAutomaticMode(task);

      // Если задача завершена, генерируем результат через AI
      if (isCompleted) {
        const aiResult = await this.generateTaskResult(task);

        if (aiResult) {
          task.result = aiResult;
          // В автоматическом режиме показываем результат кратко
          vscode.window.showInformationMessage(
            `✅ Результат задачи: ${aiResult.substring(0, 100)}${
              aiResult.length > 100 ? "..." : ""
            }`,
            { modal: false }
          );
        } else {
          // Если AI не смог сгенерировать, используем значение по умолчанию
          task.result = `Задача "${task.title}" выполнена автоматически.`;
        }
      }
    ...
    if (autoComplete) {
      // Генерируем результат и для autoComplete режима
      const aiResult = await this.generateTaskResult(task);
      if (aiResult) {
        task.result = aiResult;
      } else {
        task.result = `Задача "${task.title}" выполнена.`;
      }
      return true;
    }
  ...
  private async handleManualMode(task: Task): Promise<boolean> {
    ...
    // Если задача завершена, автоматически генерируем результат через AI
    if (isCompleted) {
      const aiResult = await this.generateTaskResult(task);

      if (aiResult) {
        task.result = aiResult;
        // Показываем сгенерированный результат пользователю
        vscode.window.showInformationMessage(
          `✅ Результат задачи сохранен:\n\n${aiResult}`,
          { modal: false }
        );
      } else {
        // Если AI не смог сгенерировать, используем значение по умолчанию
        task.result = `Задача "${task.title}" выполнена успешно.`;
      }
    }
...
2. Подробное описание задачи с ключевыми моментами реализации, учитывая архитектуру и технологии текущего проекта

Формат ответа (строго следуй):
TITLE: [полное название задачи]
DESCRIPTION: [детальное описание задачи, можно несколько строк]

Будь конкретным, ориентируйся на разработку программного обеспечения и учитывай существующую архитектуру проекта.`;
...
Будь конкретным и ориентируйся на разработку программного обеспечения.
  ...
  public async generateTaskDescriptionFromPrompt(
    ...
    return await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: "🤖 AI анализирует проект и генерирует техническое задание...",
        cancellable: true,
      },
      async (progress, token) => {
        try {
...
### Технические детали
[Описание технической реализации с учетом существующей архитектуры проекта]

### Файлы для изменения
- [Путь к файлу 1]: [Что нужно изменить]
- [Путь к файлу 2]: [Что нужно изменить]

### Критерии завершения
- [ ] [Критерий 1]
- [ ] [Критерий 2]

Будь конкретным, структурированным и ориентируйся на существующую архитектуру проекта.
...
Запрос пользователя: "${userPrompt}"`;
  ...
  private async collectProjectContext(): Promise<string> {
    ...
    try {
      ...
      context += `Тип проекта: ${packageJson.name || "неизвестно"} (${
        packageJson.description || "нет описания"
      })\n`;
      context += `Основные технологии: TypeScript, Node.js\n`;
    ...
    const tsFiles = files
      .filter((f) => f.fsPath.endsWith(".ts") || f.fsPath.endsWith(".tsx"))
      .slice(0, 20);
    if (tsFiles.length > 0) {
  ...
  /**
   * Автоматическая генерация результата выполнения задачи через AI
   * @param task Задача, для которой генерируется результат
   * @returns Сгенерированный результат или null в случае ошибки
   */
  public async generateTaskResult(task: Task): Promise<string | null> {
    const models = await vscode.lm.selectChatModels({
      vendor: "copilot",
      family: "gpt-4o",
    });
    ...
    if (models.length === 0) {
      console.log(
        "Language Model API недоступен для генерации результата задачи"
      );
      return null;
    }
    ...
    return await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: "🤖 AI генерирует результат выполнения задачи...",
        cancellable: false,
      },
      async (progress) => {
        try {
          progress.report({ message: "Анализ выполненной задачи..." });

          const prompt = `Ты — помощник для анализа выполненных задач разработки.

Задача была выполнена:
- Название: "${task.title}"
- Описание: ${task.description || "не указано"}
- Категория: ${task.category || "не указана"}
- Приоритет: ${task.priority}
${task.tag ? `- Тег: ${task.tag}` : ""}

Твоя задача: написать краткий результат выполнения задачи (2-4 предложения).

Результат должен описывать:
1. Что было сделано
2. Какие файлы/компоненты были изменены (если можно предположить из названия)
3. Какие технические решения были применены

Формат ответа: краткое описание на русском языке, без заголовков и маркеров.

Пример хорошего результата:
"Реализован компонент UserProfile с использованием React и TypeScript. Добавлены методы для загрузки и отображения данных пользователя. Обновлена документация в README.md."

Сгенерируй результат для данной задачи:`;
