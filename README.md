...
### 📝 Результаты выполнения задач (v2.11.0)

- **✅ Запись результатов**: Автоматический запрос результата при завершении задачи
  - **Интерактивный ввод**: Диалоговое окно для описания того, что было сделано
  - **Опциональное поле**: Можно пропустить, если результат очевиден
  - **Краткость**: Рекомендуется 1-2 предложения, 30-100 символов
- **👁️ Просмотр результатов**: Несколько способов посмотреть, что было сделано
  - **Tooltip**: Результат отображается при наведении на задачу в дереве
  - **Панель деталей**: Отдельный блок с зелёной рамкой в WebView панели
  - **JSON файл**: Прямой доступ к полю `result` в `.task_flow_state.json`
- **📊 Информация в промпте**: Copilot видит информацию о том, что результаты будут записаны
- **🔄 Совместимость**: Обратная совместимость со старыми задачами без поля `result`
- 📚 [Подробная документация](./docs/TASK_RESULTS.md) | [Быстрый старт](./docs/TASK_RESULTS_QUICK_START.md)
