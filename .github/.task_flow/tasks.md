...
- [ ] Интеграция отправки текста из описания задачи в ИИ для генерации технического задания [Низкий] Очередь: 1
...
- [ ] Реализация функционала для получения задач из YouTrack через API [Низкий]
  Необходимо разработать модуль, позволяющий получать задачи из системы управления проектами YouTrack.  
  Ключевые моменты реализации:  
  - Изучить документацию YouTrack API и определить необходимые эндпоинты для получения задач.  
  - Реализовать аутентификацию с использованием токена доступа.  
  - Добавить возможность фильтрации задач по параметрам (например, статус, исполнитель, проект).  
  - Обеспечить обработку ошибок и исключений при взаимодействии с API.  
  - Написать юнит-тесты для проверки корректности работы модуля.  
  - Задокументировать процесс настройки и использования модуля.

- [ ] Добавить необязательное поле "Тэг" к задачам в системе управления задачами [Низкий]
  Необходимо добавить новое поле "Тэг" к сущности задачи. Поле должно быть необязательным, но при создании или редактировании задачи пользователь должен иметь возможность указать значение.  
  - Обновить модель данных для хранения значения поля "Тэг".  
  - Добавить соответствующее поле в интерфейс создания и редактирования задачи.  
  - Обеспечить валидацию данных (например, длина тега, допустимые символы).  
  - Обновить API (если используется) для поддержки нового поля.  
  - Обновить документацию, если требуется.

- [ ] Добавить анализ контекста проекта для команды /ai перед генерацией технического задания [Низкий]
  Реализовать функциональность, позволяющую команде /ai анализировать контекст текущего проекта перед генерацией технического задания. Необходимо:  
  - Определить ключевые параметры проекта, которые должны быть учтены при анализе.  
  - Реализовать сбор и обработку данных о проекте (например, структура файлов, используемые технологии, зависимости).  
  - Интегрировать этап анализа в процесс работы команды /ai перед генерацией результата.  
  - Обеспечить обработку ошибок и уведомление пользователя в случае невозможности анализа проекта.  
  - Написать тесты для проверки корректности работы нового функционала.
